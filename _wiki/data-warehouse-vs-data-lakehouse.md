...
The podcast archive converges on a warehouse-first definition when the main
consumer is trusted analytics. Kwong's modern-stack episode connects
...
The archive converges on a lakehouse definition when the storage boundary has
to stay open. Albertsson starts from object storage and raw data. He then adds
...
ML and Spark-style processing make the case stronger. Vendor lock-in can be
another first-order constraint.
...
A practical split is to keep raw events and long-lived history in lakehouse
storage. Finance, growth, BI, and activation can still consume
...
work. Otherwise it has a data lake with a table format, not a usable lakehouse.
...
Cost can therefore point either way. A warehouse can be the cheaper choice when
one managed SQL system serves the consumers and FinOps practices control usage.
A lakehouse can be the better choice when open storage, multiple engines, or
long-lived raw history avoids expensive copying or product lock-in. The archive
supports making that call from requirements rather than from architecture
labels
([Modern Data Engineering Trends]({{ '/podcasts/trends-in-modern-data-engineering/' | relative_url }}),
44:42 and
[FinOps for Data Engineers]({{ '/podcasts/finops-for-data-engineers/' | relative_url }}),
...
stronger. ML workloads and lower vendor lock-in do too.
...
and operational tools can push it there too.
