...
It also connects [evaluation]({{ '/wiki/evaluation/' | relative_url }}) and
[production]({{ '/wiki/production/' | relative_url }}). Tests encode known
expectations before release. Monitoring, [data observability]({{ '/wiki/data-observability/' | relative_url }}),
and [model monitoring]({{ '/wiki/model-monitoring/' | relative_url }}) catch
behavior the team didn't know how to encode yet.

For data teams, testing usually means data-quality assertions, transformation
checks, and realistic test data. It also means regression suites and CI/CD
gates. For ML and AI teams, it means offline evaluation sets and prompt evals.
It also means RAG evals, tool-call tests, and production feedback loops.
...
gives the analytics-engineering version in
...
LLM and agent guests use the word evaluation more often than testing, but they
...
At 53:20-56:02, she frames agent checks as software tests with mocked tools and
integration tests. She also includes regression tests and outcome assertions.
...
That makes analytics-engineering testing different from a one-off analyst
query. The test belongs near the model definition, runs with the transformation
flow, and tells the team whether a shared business definition can still be
trusted. Documentation and peer review matter too. Perez Mola ties good SQL,
tests, guidelines, and review practices to the analytics engineer role at
42:27-44:12.
...
and [reproducibility]({{ '/wiki/reproducibility/' | relative_url }}). A data
...
retrieval before polishing formatting. This connects testing to
...
These pages cover adjacent practices for delivery, evaluation, and production
confidence:
