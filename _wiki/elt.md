...
receives raw or lightly processed data first. The team then builds typed,
...
That puts ELT near [data pipelines]({{ '/wiki/data-pipelines/' | relative_url }})
...
That makes Airbyte an ingestion tool in this page's vocabulary, not the owner
of the warehouse transformation layer.
...
[Airflow]({{ '/wiki/orchestration/' | relative_url }}) belongs at the scheduling
...
places it next to catalogs, metadata, and lineage. His point for ELT is that
...
That same flexibility can create unused raw data, unclear ownership, and
inconsistent definitions if teams don't maintain the warehouse layers.
...
That connects ELT to [DataOps]({{ '/wiki/dataops/' | relative_url }}) because
teams need versioned code, tests, and lineage. They also need observability and
repeatable runs, not only a load-first diagram.
...
[Gloria Quiceno]({{ '/people/gloriaquiceno/' | relative_url }}) connects the
...
warehouse-specific SQL, clean data, and quality checks. That episode is less
...
Kwong's modern analytics argument is to load source detail, keep the warehouse
flexible, and let analytics teams transform with SQL and dbt. She still warns
that raw layers need governance, cleanup, and data mart boundaries
([modern stack episode at 7:57-18:47 and 43:02-45:59]({{ '/podcasts/data-engineering-tools-modern-data-stack/' | relative_url }})).

Perez Mola argues from the analytics engineer's desk. She focuses less on the
ELT acronym and more on models, tests, and DAGs. She also emphasizes bad data,
schema changes, and raw-input limits
([analytics engineering episode at 36:44-48:36]({{ '/podcasts/analytics-engineer-skills-tools/' | relative_url }})).

Tuli draws a sharper boundary around ingestion, and she doesn't treat every
pre-load action as business transformation. Deduplication, ordering guarantees,
and masking can happen near ingestion. Business modeling happens later with
warehouse entities and use cases
...
Jain argues from scale and leadership because fixed target models became too
tightly coupled as use cases grew. His team moved from ETL to ELT, but they
kept traditional and flat models. They also used a data lake, lineage, and
consumer-facing exposure paths
...
Brudaru updates the stack boundary. For him, ELT now sits alongside open table
formats, catalogs, and metadata. Orchestration choices and requirements-led tool
selection also matter
([modern data engineering trends at 18:17-35:37 and 41:06-44:42]({{ '/podcasts/trends-in-modern-data-engineering/' | relative_url }})).
That makes ELT a durable workflow structure rather than a fixed list of vendors.
