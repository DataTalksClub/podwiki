...
Future additions should stay observability-specific:

- Add evidence when an episode names observable data signals, incident
  diagnosis, lineage, SLAs, or alerting. Runbooks, ownership, false positives,
  and root cause workflows also fit.
- Keep generic "data quality matters" material on
  [Data Quality and Observability]({{ '/wiki/data-quality-and-observability/' | relative_url }})
  unless the episode includes observability-specific mechanisms.
- When adding model-monitoring evidence, note whether the issue is a data signal,
  model behavior signal, or both.
